...
export function buildTransactionRows(transactions, userId) {
  return normalizeTransactions(safeArray(transactions)).map((transaction) =>
    mapTransactionToRow(transaction, userId),
  );
}
