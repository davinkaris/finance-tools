...
function isEligibleForMatching(transaction) {
  return !transaction?.matchedTransactionId;
}
...
export function detectMoveMoney(transactions, accounts = []) {
...
export function runTransactionMatching(transactions, accounts = []) {
  const moveMoneyResult = detectMoveMoney(transactions, accounts);
